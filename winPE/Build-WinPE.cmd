...
REM manual notes
REM     логика использования скрипта получилась крайне запутання :)
REM     так вышло из-за перехода от ввода консольных команд к скрипту и постепенного наращивания его функционала
REM 
REM     сейчас скрипт реализует три подхода (уровня) при сборке итогового iso-файла, разной степени длительности:
REM         1 - самый долгий - сборка с чистого листа (параметр "clear", полуфабриката clear.wim ещё нет)
REM         все пакеты интегрируются в фабричный kit`овский boot.wim (это достаточно длительная процедура),
REM         далее сюда же добаляется Wenix (намного быстрее)
REM 
REM         2 - самый средний - если при первом подходе закоментировать интеграцию Wenix`а, Far`а и проч.,
REM         то получим ПОЛУФАБРИКАТ clear.wim, в который можно снова и снова включать свежие версии Wenix`а
REM 
REM         3 - самый быстрый - пересборка только iso-файла (параметр "iso"),
REM         boot.wim остаётся прежним (старым и со старой версией Wenix`а),
REM         новая версия Wenix`а копируется в <iso-root>:\.IT\PE\Wenix
REM 
REM     так вот, по-идее архитектурно можно и нужно переделать скрипт так,
REM     чтобы подход выбирался при передаче параметра
REM 
REM     для этого потребуется автоматизировать полуфабрикацию между
REM     этапом добавления пакетов и этапом добавления Wenix`а + ПО
